...
> **注意：** `HOT_SIZE` / `MAX_CONCURRENT` 等数字环境变量由纯 Go 的 `render-config` 写入 `config.yaml`。旧版 Python entrypoint 在 `HOT_SIZE=3000` 时会因 `re` 反向引用把 `hot_size` 行写成 `X00` 导致容器 crash loop。
